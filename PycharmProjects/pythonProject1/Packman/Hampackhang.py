...
def move_ghost():
    ...
    for i in range(1):
        ghost = ghost1
        ghost.x = random.randint(0,SCREEN_WIDTH)
        ghost.y = random.randint(0,SCREEN_HEIGHT)
        dx = random.randint(-5,5)
        dy = random.randint(-5,5)
        Ghost.append((ghost,dx,dy))

    for (ghost,dx,dy) in Ghost:
         ghost.x += dx
         ghost.y += dy
...
def main():
    global resized_tree, resized_pacman, resized_dot, resized_zero, resized_ghost1
